export function StatsSection() {
  ...
  return (
    <section className="py-16 px-6" style={{ backgroundColor: "#F1F5F9" }}>
      <div className="max-w-5xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.label} className="text-center">
            <p className="text-5xl font-black leading-none mb-2" style={{ color: "#2F4F4F" }}>{s.value}</p>
    ...
    <section className="py-24 px-6" style={{ backgroundColor: "#FFFFFF" }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          <h2 className="text-4xl font-black leading-tight" style={{ color: "#2F4F4F", letterSpacing: "-0.02em" }}>People love EzyGo</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reviews.map((r) => (
            <div key={r.name} className="rounded-2xl p-6 flex flex-col gap-4 transition-all hover:-translate-y-1 hover:shadow-lg" style={{ border: "1px solid #E2E8F0", backgroundColor: "#F8FAFC" }}>
              ...
              <p className="text-sm leading-relaxed flex-1" style={{ color: "#475569" }}>&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center gap-3 pt-2 border-t" style={{ borderColor: "#E2E8F0" }}>
                ...
                <div>
                  <p className="text-sm font-bold" style={{ color: "#2F4F4F" }}>{r.name}</p>
                  <p className="text-xs" style={{ color: "#94A3B8" }}>{r.role}</p>
                </div>
              </div>
            </div>
          ))}
...
export function Footer() {
  return (
    <footer className="py-12 px-6 border-t" style={{ backgroundColor: "#0D1F1F", borderColor: "rgba(61,102,102,0.3)" }}>
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-8 mb-10">
          <div className="max-w-xs">
            ...
            <p className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>Fast, reliable courier delivery across South Africa. One flat fee — no surprises.</p>
          </div>
            ...
            {[
              ...
              <div key={col.heading}>
                ...
                <ul className="space-y-2">
                  {col.links.map((l) => (
                    <li key={l}><a href="#" className="transition-opacity hover:opacity-100" style={{ color: "rgba(255,255,255,0.4)" }}>{l}</a></li>
                  ))}
                </ul>
              </div>
            ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-8 border-t gap-4" style={{ borderColor: "rgba(61,102,102,0.3)" }}>
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>© {new Date().getFullYear()} EzyGo. All rights reserved. Built in South Africa 🇿🇦</p>
          <div className="flex items-center gap-4">
            {["PayFast", "SSL Secured"].map((b) => (
              <span key={b} className="text-xs px-2.5 py-1 rounded-lg font-medium" style={{ backgroundColor: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)", border: "1px solid rgba(255,255,255,0.08)" }}>{b}</span>
            ))}
          </div>
        </div>
      </div>
